...
// 72 partidos, fase de grupos. Horarios en hora del centro de México (UTC-6).
...
const STORAGE_KEY = "quiniela_mundial_2026_v3";
...
// ─── DATE HEADER (dentro de listas de partidos) ───────────────────────────────

const DateHeader = ({ dateStr }) => (
  <div style={{ fontSize:12, fontWeight:700, color:"#e94560", textTransform:"uppercase", letterSpacing:1, padding:"12px 0 4px", borderTop:"1px solid rgba(233,69,96,0.15)", marginTop:6 }}>
    📅 {formatDate(dateStr)}
  </div>
);

// ─── SCREENS (fuera del componente principal para evitar remount) ──────────────
    ...
    <div style={{ maxWidth:800, margin:"0 auto", padding:"20px 16px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:8 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:"#888" }}>{participants.length} participante(s) registrados</div>
        </div>
        {savedMsg && <div style={{ background:"rgba(27,127,74,0.2)", border:"1px solid #1b7f4a", color:"#4ade80", borderRadius:6, padding:"3px 12px", fontSize:12, fontWeight:700 }}>{savedMsg}</div>}
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
          {[1,2,3].map(j => {
            const startDates = {1:"11–17 Jun", 2:"18–23 Jun", 3:"24–27 Jun"};
            return (
              <div key={j} style={{ flex:1, background:openJornadas[j]?"rgba(27,127,74,0.15)":"rgba(233,69,96,0.08)", border:"1px solid "+(openJornadas[j]?"#1b7f4a":"#333"), borderRadius:10, padding:"14px 16px", textAlign:"center", minWidth:120 }}>
                ...
                <div style={{ fontSize:10, color:"#666", marginBottom:6 }}>{startDates[j]}</div>
...
export default function QuinielaMundial() {
  ...
  useEffect(() => {
    loadState().then(s => {
      if (s) {
        ...
        setOpenJornadas(s.openJornadas||{1:false,2:false,3:false});
        setAdminPass(s.adminPass||"admin123");
      }
      setLoaded(true);
    });
  }, []);
  ...
  const persist = (r, p, oj, ap) => saveState({results:r, participants:p, openJornadas:oj, adminPass:ap});
  ...
  const handleResultChange = useCallback((matchId, field, val) => {
    setResults(prev => {
      const nr = {...prev, [matchId]:{...(prev[matchId]||{}), [field]:val}};
      saveState({results:nr, participants, openJornadas, adminPass});
      return nr;
    });
  }, [participants, openJornadas, adminPass]);

  const toggleJornada = useCallback((j) => {
    setOpenJornadas(prev => {
      const nj = {...prev, [j]:!prev[j]};
      saveState({results, participants, openJornadas:nj, adminPass});
      flash(nj[j]?`✅ Jornada ${j} abierta`:`🔒 Jornada ${j} cerrada`);
      return nj;
    });
  }, [results, participants, adminPass]);

  const handleChangePass = useCallback(() => {
    if (!newAdminPass.trim()) return;
    setAdminPass(newAdminPass);
    saveState({results, participants, openJornadas, adminPass:newAdminPass});
    setNewAdminPass(""); flash("✅ Contraseña actualizada");
  }, [newAdminPass, results, participants, openJornadas]);
  ...
  const handleNewParticipant = useCallback(() => {
    ...
    const nps = [...participants, np];
    setParticipants(nps);
    saveState({results, participants:nps, openJornadas, adminPass});
    setParticipantName(""); setActiveParticipantId(np.id); setCurrentPreds({}); setScreen("participant");
  }, [participantName, participants, results, openJornadas, adminPass]);
  ...
  }, [activeParticipantId, currentPreds, results, openJornadas, adminPass]);
  ...
  if (!loaded) return (
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ color:C.red, fontSize:22 }}>Cargando quiniela…</div>
    </div>
  );
